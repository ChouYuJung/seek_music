...
class NewReleaseCategories:
    PATH = "new-release-categories"
    PATH_ID = "new-release-categories/{category_id}"
    PATH_ID_ALBUMS = "new-release-categories/{category_id}/albums"
    ...
    def list_albums(
        ...
        url = str(
            base_url.with_path(
                join_paths(
                    base_url.path, self.PATH_ID_ALBUMS.format(category_id=category_id)
                )
            )
        )
        headers = self.parent.headers
